...
namespace Car2_Control
{
        class Car2Control : public Common_Control::CommonControl
        {
        public:
                Car2Control(const std::string &robot_name, const std::string &odom_topic, const std::string &robot_topic, const std::string &pose_pub_topic)
                ...
                void Init()
                {
                        CommonControl::Init();
                        ros::NodeHandle nh;
                        car1_pose_sub = nh.subscribe("car1_pose", 1, &Car2Control::car1PoseCallback, this);
                        car1_state_sub = nh.subscribe("car1_state", 1, &Car2Control::car1StateCallback, this); // 订阅机器人1的状态
                        marker1_pub = nh.advertise<visualization_msgs::Marker>("visualization1_marker", 1);
                        dis_pub=nh.advertise<std_msgs::Float64>("dis_car1_car2",1);
                }
                ...
                void Run()
                {
                        ...
                        while (ros::ok())
                        {
                                ...
                                if (car1_path_vector.empty())
                                {

                                        ROS_INFO("car1_path_vector is empty,waiting");
                                ...
                                dis_msg.data=dis_car1_car2;
                                dis_pub.publish(dis_msg);
                                ROS_INFO("dis_car1_car2=%f",dis_car1_car2);
        ...
        private:
                ros::Subscriber car1_pose_sub; // 订阅机器人1的路径点话题
                ros::Subscriber car1_state_sub; // 订阅机器人1的状态话题
                ros::Publisher dis_pub;
                Path_Type car1_path_vector;    // 存储机器人1的路径点
                ros::Publisher marker1_pub;
                double dis;
                double dis_car1_car2;
                Common_Control::StateValue car1_state; // 机器人1的状态

                void Running_State()
                {
                        // 获取当前目标路径点
                        path_type current_goal = car1_path_vector[current_index];
                        ...
                        case Common_Control::RunStateValue::Goto:
                                ...
                                */
                                ...
                                Goto_Goal(car1_path_vector[current_index], 1, 0.02 + 1.0 * (dis - 9.0), 0.002, 1.0);
                        ...
                        case Common_Control::RunStateValue::Tracking:
                                ...
                                */
                                if (current_index < car1_path_vector.size())
                                {
                                        Tracking_Path_pure_pursuit(car1_path_vector, 1, 0.02 + 1 * (dis - 9.0), 0.08);
                                }
                                else
                                {
                ...
                void car1PoseCallback(const project_truck::RobotPose::ConstPtr &msg)
                {
                        // 将接收到的路径点添加到路径向量中
                        car1_path_vector.push_back({{msg->x, msg->y}, msg->yaw});
                        dis = sqrt(pow(msg->x - robot_pose.x, 2) + pow(msg->y - robot_pose.y, 2));
                        // 可视化机器人1的路径点
                        Visual_Path(car1_path_vector);
                }
...
{
        ros::init(argc, argv, "car2_control");

        std::string robot_name = "car2";
        std::string odom_topic = "/current_pose";
        std::string robot_topic = "/underpan_speed_1";
        std::string pose_pub_topic = "car2_pose";

        Car2_Control::Car2Control car2(robot_name, odom_topic, robot_topic, pose_pub_topic);
        car2.Init(); // 初始化
        car2.Run();  // 运行
