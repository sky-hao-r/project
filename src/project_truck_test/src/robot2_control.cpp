...
namespace Robot2_Control
{
        ...
        public:
                ...
                void Init()
                {
                        ...
                        robot1_state_sub = nh.subscribe("robot1_state", 1, &Robot2Control::Robot1StateCallback, this); // 订阅机器人1的状态
        ...
        private:
                ...
                Running_State()
                {
                        ...
                        case Common_Control::RunStateValue::Goto:
                                ...
                                if (robot_type)
                                {
                                        Goto_Goal(robot1_path_vector[current_index], 0.05, 0.1 + 1.0 * (dis - 3.0), 1.0, 1.0);
                                }
                                else
                                {
                                        Goto_Goal(robot1_path_vector[current_index], 0.3, 0.1 + 1.0 * (dis - 3.0), 1.0, 1.0);
                                }
                        ...
                        case Common_Control::RunStateValue::Tracking:
                                ...
                                if (current_index < robot1_path_vector.size())
                                {
                                        ...
                                        Tracking_Path_pure_pursuit(robot1_path_vector, 1, 0.1 + 1 * (dis - 3.0));
                                }
                                else
                                {
...
{
        ros::init(argc, argv, "robot2_control");

        std::string robot_name = "robot2";
        // std::string odom_topic = "/robot2/current_pose";
        // std::string robot_topic = "/robot2/cmd_vel";
        std::string odom_topic = "/current_pose_1";
