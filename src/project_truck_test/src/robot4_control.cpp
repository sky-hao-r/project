...
namespace Robot4_Control
{
        class Robot4Control : public Common_Control::CommonControl
        {
        public:
                Robot4Control(const std::string &robot_name, const std::string &odom_topic, const std::string &robot_topic, const std::string &pose_pub_topic)
                ...
                void Init()
                {
                        ...
                        robot3_pose_sub = nh.subscribe("robot3_pose", 1, &Robot4Control::Robot3PoseCallback, this);
                        robot1_state_sub = nh.subscribe("robot1_state", 1, &Robot4Control::Robot1StateCallback, this); // 订阅机器人1的状态
                        marker3_pub = nh.advertise<visualization_msgs::Marker>("visualization3_marker", 1);
                }
                ...
                void Run()
                {
                        ...
                        while (ros::ok())
                        {
                                ...
                                if (robot3_path_vector.empty())
                                {
                                        ROS_INFO("robot3_path_vector is empty,waiting");
                                        rate.sleep();
                                        continue;
                                }
                                ...
                                case Common_Control::StateValue::Idle:
                                        ...
                                        ROS_INFO("Robot4 is transitioning to Run state...");
                                        // 立即进入下一个状态
                                        break;
        ...
        private:
                ros::Subscriber robot3_pose_sub; // 订阅机器人3的路径点话题
                ros::Subscriber robot1_state_sub; // 订阅机器人1的状态话题

                Path_Type robot3_path_vector;    // 存储机器人3的路径点
                ...
                {
                        // 获取当前目标路径点
                        path_type current_goal = robot3_path_vector[current_index];

                        // 检查运行状态
                        switch (Run_State)
                        {
                        case Common_Control::RunStateValue::Goto:
                                ROS_INFO("Robot4 is going to the first goal point.");
                                // 前往第一个路径点
                                if (robot_type)
                                {
                                        Goto_Goal(robot3_path_vector[current_index], 0.05, 0.3 + 1.0 * (dis - 2.0), 1.0, 1.0);
                                }
                                else
                                {
                                        Goto_Goal(robot3_path_vector[current_index], 0.05, 0.3 + 1.0 * (dis - 2.0), 1.0, 1.0);
                        ...
                        case Common_Control::RunStateValue::Tracking:
                                ROS_INFO("Robot4 is tracking the path.");
                                // 轨迹跟踪
                                if (current_index < robot3_path_vector.size())
                                {
                                        // Tracking_Path_stanley(robot1_path_vector);
                                        Tracking_Path_pure_pursuit(robot3_path_vector,0.05,0.1);
                                }
                                else
                                {
                                        ROS_INFO("Robot4 has completed tracking the path.");
                                        Robot_State = Common_Control::StateValue::Idle;
                                }
                                break;
                        case Common_Control::RunStateValue::Stop:
                                ROS_INFO("Robot4 is stopping.");
                                Stop(); // 停止机器人
                                break;
                ...
                void Robot3PoseCallback(const project_truck::RobotPose::ConstPtr &msg)
                {
                        // 将接收到的路径点添加到路径向量中
                        robot3_path_vector.push_back({{msg->x, msg->y}, msg->yaw});
                        dis = sqrt(pow(msg->x - robot_pose.x, 2) + pow(msg->y - robot_pose.y, 2));
                        // 可视化机器人1的路径点
                        Visual_Path(robot3_path_vector);
                }
...
{
        ros::init(argc, argv, "robot4_control");

        std::string robot_name = "robot4";
        std::string odom_topic = "/robot4/current_pose";
        std::string robot_topic = "/robot4/cmd_vel";
        std::string pose_pub_topic = "robot4_pose";

        Robot4_Control::Robot4Control robot4(robot_name, odom_topic, robot_topic, pose_pub_topic);
        robot4.Init(); // 初始化
        robot4.Run();  // 运行
